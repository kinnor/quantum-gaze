...
9. **Brand markup**: navbar and footer use `<a class="brand"><img src="images/QGS-Mark-128.png" class="brand-mark"><span class="brand-wordmark">Quantum Gaze<small>Software Inc.</small></span></a>` — the Q-eye mark is the logo (owner decision 2026-08-29). Do not reintroduce `QGS-Logo-EA.svg`; `og:image` stays `images/og-card.png` (built from the mark); the footer is dark (see the "Footer" override block in `luxury-enterprise.css`).
