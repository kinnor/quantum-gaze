...
8. **The `<!-- qg:head:start --> … <!-- qg:head:end -->` block** in every `<head>` (favicon set, Open Graph, Twitter card, theme-color; JSON-LD on index). `og:image` must stay `images/og-card.png` (1200×630 PNG) — never an SVG. No `hreflang`: all languages share one URL, `og:locale:alternate` is the correct signal.
9. **Brand markup** (owner decision 2026-08-30): navbar and footer use the approved **Neural Aperture "Q" vector lockup** `<a href="index.html" class="brand"><img src="images/QGS-Logo-EA.svg?v=<asset-version>" class="brand-lockup" height="48"></a>` (footer: `brand brand-footer`, height 56). Version the SVG URL like css/js. Favicons: `/favicon.png` (180) + `favicon.ico` + `images/favicon-*.png`; app icon `images/quantum-gaze-icon.png`; `og:image` stays `images/og-card.png`. Brand tokens `--obsidian/--cyan/--sapphire/--titanium` live in `luxury-enterprise.css` `:root`. The footer is dark (see the "Footer" override block).
